...
- NEVER commit vault contents, credentials, or gitignored material. Several
  folders (vault copies, testing dirs) are gitignored BY DESIGN — never add
  gitignored material to git. This governs commits, not reads: L44 (equal
  access) entitles every house to read the vault, reports, and memory files;
...
- Commit continuously to git/GitHub. Small commits, clear messages. This
  governs granularity within a run; L46 (one run, one commit) governs the
  boundary between runs — main is clean and committed before the next run
  starts.
...
and README.md are the only markdown files that stay at repo root
(`AGENTS.md`/`QWEN.md` are the repo-owned house stubs built 2026-09-13,
docs/00 - Project/PROJECT-LEDGER.md:1316). (The lone exception:
...
text instruction. `/clear` at task boundaries; prompts carry paths and
rulings, not file contents.

**Capture hygiene and DevDocs authorship (ruled 2026-09-10):** reviewer
captures and Codex session logs never enter a commit; `_inflight/*.log` is
gitignored. DevDocs prose stays agent-authored; the symbol-check gate is
the acceptance test, not a generator.
