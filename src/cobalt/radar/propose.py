...
from cobalt.vaultwrite import AT_END, VaultWriteStore, VaultWriter
...
def derive_screens(text: str) -> tuple[list[ScreenBlock], list[dict[str, str]]]:
    ...
    for index, match in enumerate(matches):
        ...
        if active is None:
            raise ProposalRefused(f"{match.group(2)}: missing prose field Active")
...
def _render_screen(block: ScreenBlock, evidence: dict[str, str]) -> str:
    dumped = yaml.safe_dump(block.model_dump(exclude_none=True), sort_keys=False).strip()
    sources = "\n".join(f"# from: {json.dumps(value)}" for value in evidence.values())
    return f"{sources}\n```yaml\n{dumped}\n```"
...
def screens_propose(args) -> None:
    ...
    artifact = build_artifact(
        "screens", target, hashlib.sha256(target.read_bytes()).hexdigest(),
...
def apply(args) -> None:
    ...
    if artifact["kind"] == "lists":
        ...
        if "```yaml" in target.read_text(encoding="utf-8"):
