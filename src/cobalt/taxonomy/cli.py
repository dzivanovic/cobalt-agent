"""`cobalt taxonomy …` — the vault-backed trade_def commands.
    ...
    cobalt taxonomy assumed write --from <rows.yaml> --dry-run | --apply
    cobalt taxonomy tunables --assumed     (read-only)
...
def add_parser(sub) -> None:
    ...
    assumed = gsub.add_parser(
        "assumed",
        help="The assumed-defaults note (FINAL §8): write it through the L28 writer.",
    )
    asub = assumed.add_subparsers(dest="assumed_command", required=True)
    write = asub.add_parser("write", help="Create-if-absent + upsert the tunables:assumed unit.")
    write.add_argument("--from", dest="rows_file", required=True,
                       help="A TunableRegistry-shaped YAML (`tunables:` rows, source assumed|ruling).")
    write.add_argument("--dry-run", action="store_true")
    write.add_argument("--apply", action="store_true")
    write.set_defaults(func=cmd_assumed_write)

    tunables = gsub.add_parser(
        "tunables", help="Read-only: the tunable rows as the radar would resolve them.",
    )
    tunables.add_argument("--assumed", action="store_true", required=True,
                          help="List every assumed row and every engine hole still null (L10 dry-run).")
    tunables.set_defaults(func=cmd_tunables_assumed)


# ---------------------------------------------------------------------
# assumed defaults (setups one build STEP-2; FINAL §7 point 3, §8)
# ---------------------------------------------------------------------


def assumed_note_text(rows: list[dict]) -> str:
    """The whole `Assumed Defaults.md` note, its ONE marker-bounded unit
    holding `rows` (an empty list writes the unit empty)."""
    from cobalt.vaultwrite.markers import render_section, render_unit

    from .vault_loader import ASSUMED_SECTION, ASSUMED_UNIT
    ...
    lines = [
        ...
        "",
        *render_section(ASSUMED_SECTION, render_unit(ASSUMED_UNIT, _assumed_body(rows))),
    ]
    return "\n".join(lines) + "\n"


def _assumed_body(rows: list[dict]) -> str:
    import yaml

    return "```yaml\n" + yaml.safe_dump({"tunables": list(rows)}, sort_keys=False).rstrip("\n") + "\n```"


def write_assumed(vault_root: Path, rows_file: Path, *, store, dry_run: bool = False):
    """THE writer of the assumed note — a Cobalt command over the EXISTING
    L28 `VaultWriter` (`create_if_absent`, then `upsert_unit`): marker-
    bounded, three-way merged (a line he edited wins), audited in
    `vault_writes`. The rows validate through `TunableRegistry` first and
    must read `source` assumed or ruling. Returns both write results, each
    with its unified diff."""
    import yaml

    from .tunables import TunableRegistry, TunableSource
    from .vault_loader import ASSUMED_NOTE, ASSUMED_SECTION, ASSUMED_UNIT

    raw = yaml.safe_load(Path(rows_file).read_text(encoding="utf-8"))
    registry = TunableRegistry(**raw)
    for row in registry.tunables:
        if row.source not in (TunableSource.ASSUMED, TunableSource.RULING):
            raise SystemExit(f"{rows_file}: row {row.key!r} has source {row.source.value!r}; assumed|ruling only")
    rows = [row.model_dump(mode="json", exclude_none=True) for row in registry.tunables]
    writer = VaultWriter("taxonomy-assumed", store=store, dry_run=dry_run)
    path = Path(vault_root) / ASSUMED_NOTE
    created = writer.create_if_absent(path, assumed_note_text([]))
    upserted = writer.upsert_unit(path, ASSUMED_SECTION, ASSUMED_UNIT, _assumed_body(rows))
    return [created, upserted]


def cmd_assumed_write(args: argparse.Namespace) -> None:
    from cobalt.vault import resolve_vault_path

    dry_run = _require_mode(args, "assumed write")
    for result in write_assumed(resolve_vault_path(), Path(args.rows_file), store=VaultWriteStore(),
                                dry_run=dry_run):
        print(result.report())


class AssumedLine:
    """One line of `cobalt taxonomy tunables --assumed`."""

    def __init__(self, *, key, state, unit, scope, value, consumers, source_page=""):
        self.key, self.state, self.unit, self.scope = key, state, unit, scope
        self.value, self.consumers, self.source_page = value, consumers, source_page

    def render(self) -> str:
        return (f"{self.state:<8} {self.key:<40} unit={self.unit} scope={self.scope} value={self.value} "
                f"consumers={self.consumers} page={self.source_page or '—'}")


def assumed_report(engine: dict, vault) -> list[AssumedLine]:
    """Every row whose RESOLVED source is `assumed`, and every engine hole
    still null — key, unit, scope, consumers. The source page stays EMPTY in
    the repo: it lives in his companion (L32). Read-only."""
    from .loader import merge_tunables
    from .tunables import TunableSource

    merged = merge_tunables(dict(engine), vault.tunable_rows)
    out = []
    for key, row in sorted(merged.items()):
        if row.source is TunableSource.ASSUMED:
            ...
            continue
        out.append(AssumedLine(key=key, state=state, unit=row.unit.value, scope=row.scope, value=row.value,
                               consumers=list(row.consumers)))
    return out


def cmd_tunables_assumed(args: argparse.Namespace) -> None:
    from .loader import load_tunables

    lines = assumed_report(load_tunables().by_key, load_vault_trade_defs())
    for line in lines:
        print(line.render())
    print(f"\n{sum(l.state == 'assumed' for l in lines)} assumed row(s), "
          f"{sum(l.state == 'hole' for l in lines)} engine hole(s) still null. Writes: none.")
