...
## 10. ORIGINAL INTENT (appended 2026-08-21; recorded separately — does not alter §1/§7 verdicts)

Intent sources available (tracked only — vault ADR/PRD notes gitignored and not read; `cobalt_master_context.txt` absent from disk): git commit subjects/bodies; System-Manifest/Security-Architecture/ADR text in `dev_utils/generate_constitution.py`; PRD-001 in `dev_utils/create_prd.py`; task notes in `dev_utils/create_missing_tasks.py`; module/test docstrings naming sprint phases; `COBALT-REQUIREMENTS.md`. Where nothing documents a component, intent is marked UNVERIFIED.

| Component | File | Verdict (from §1/§7) | ORIGINAL INTENT (documentation trail) |
|---|---|---|---|
| `BrowserTool` plain URL→text | `browser.py:605-771` | RETAIN | Initial commit `a366e9e` (02-08) "Core agent with Search and Browser tools"; `b01b96f` "Added Finance Tool and Browser capability". System Manifest: Scout department "Web Browsing & Due Diligence", stack `playwright`/`beautifulsoup4` (`generate_constitution.py:140-143,178`). |
| Pre-Flight `llms.txt` / `Accept: text/markdown` dual path | `browser.py:481-565` | RETAIN | Module docstring "Pre-Flight Protocol (Dual-Path routing with llms.txt/fast path)" (`browser.py:13`); test class "Phase 2 — Watcher Daemon" (`tests/test_universal_extractor.py`) → commit `333c2c3` (02-28). Rationale (cheap markdown before Playwright): docstring only — no ADR. |
| Action DSL (`click/type/maps/extract/inject_credentials`), `AOMExtractor`, `Maps` | `browser.py:38-74,119-360`, `aom.py`, `maps.py` | KILL-candidate | `b090110` (02-27) "implement AOM agentic browser loop, zero-trust security, and pgvector fast path"; `aom.py:1-14` docstring (CDP DOM snapshot → numeric IDs for stable referencing); `maps.py:1-13` ("map numeric IDs to ElementHandle… refresh on navigation"). Intent = LLM-driven clicking by element id; never completed end-to-end (H-4). |
| Domain whitelist (`allowed_domains`, `is_url_allowed`) | `aom.py:46-85`, `config.yaml:135-140` | RETAIN logic / BROKEN enforcement | `b090110` "zero-trust security"; Security Architecture §5 "The Airlock: Scout isolated in a 'Dirty' VLAN… can only write to a sanitized Drop Zone" (`generate_constitution.py:214-218`) — whitelist is the code-level remnant of that isolation idea. |
| Vault credential injection (`inject_credentials`) | `browser.py:276-360` | RETAIN | `5becd1e`/`b090110`; Security Architecture §3 (secrets never in code/env; RAM-only). |
| Browser-side FastPath methods | `browser.py:362-479` | KILL-candidate | `b090110` "pgvector fast path"; "Phase 3" per `tests/test_browser_fast_path.py:2`; CLAUDE.md "FastPathCache for Playwright script reuse". Never wired into `run()` — UNVERIFIED whether intentionally deferred. |
| `UniversalExtractor` + `compute_delta` | `extractor.py` | BROKEN-FRICTION | `333c2c3` "watcher daemon, graph memory"; module docstring "LLM-powered extraction of graph entities… Delta engine for computing new edges" (`extractor.py:1-16`); test "Phase 2 — Watcher Daemon". Intent: detect *new* entities on watched pages and alert. |
| `DaemonTool` / watcher jobs / Mattermost alert | `daemon.py` | KILL-candidate (as built) | `333c2c3`; docstring "Scheduled Web Monitoring System… Mattermost interrupt system for alerts on new entities" (`daemon.py:1-12`); task 33 "Incoming Webhooks for alerts (Trade Signals)"; System Manifest Scout "Sentiment Analysis (News/Social)". Precursor of Requirements §5 news agents — but generic URL watching, not news feeds. |
| `FinvizExtractor` (Playwright scraper, "Recon Scout") | `finviz_extractor.py` | KILL-candidate | `8a69874` (03-17) "align PRDs and task board with Unified MoE and Recon Scout sprint"; docstring "Finviz Recon Scout — Elite Screener Extraction Skill… logs into Finviz Elite using Zero-Trust Vault credentials" (`:1-24`). Requirements §5 "Playwright scraping where no API/MCP exists" — superseded for Finviz by the export API (`finviz_api.py` docstring: "Bypasses browser automation entirely… 10-50x faster than Playwright-based extraction", `:4-5,59`). |
| `FinvizApiClient` ("Macro Engine") | `finviz_api.py` | RETAIN | `cc14caa` (04-07) "Research pipeline stabilization"; docstrings "Finviz API Client — The Macro Engine" (`:2`), `dev_utils/live_run_finviz.py:2`; Requirements §7 "Finviz Elite: scanners (functional today), news, single-stock OHLC". |
| `ScannerOrchestrator` + `configs/scanners.yaml` ("Universal Abstract Screener") | `scanner_orchestrator.py`, `scanners.yaml` | RETAIN → fix | `cc14caa`; `dev_utils/init_5_pillar_schema.py` "ready for Task 2 (Finviz Orchestrator)"; `live_run_dynamic_scanners.py:1-12` "Universal Abstract Screener… YAML-driven filter configuration"; `scanners.yaml:1-2`. PRD-001 Story A: "Cobalt to scan the market for 'In Play' stocks and identify the specific Strategies (from my Playbook) that apply… a curated list of opportunities" (`create_prd.py:55-59`); Requirements §1/§5 premarket + real-time agents. The `schedule:` blocks express the intended cadence (04:00–10:00 / 09:30–16:00) that nothing executes yet. |
| `MetadataEnricher` | `enrich_metadata.py` | RETAIN | `cc14caa`; docstring "injecting hard financial data into the database… callable by other agents or scheduled orchestrators" (`:2-3,51-52`). Requirements §8 Tier-1 (float, short interest). |
| `SemanticTagger` + `themes` / `sync_taxonomy` | `semantic_tagger.py`, `sync_taxonomy.py` | Pass 4 | `cc14caa`; docstrings "Semantic Tagging Drip Engine — Step 3 (Recursive Architecture Update)", "Master Taxonomy Sync (Recursive Architecture Restored)"; 5-Pillar `themes` "Taxonomy of market themes/motifs (hierarchical & AI Ready)" (`schema.sql:26`). Intent: theme/sector tagging for "stocks in play" context and the morning plan. |
| `dev_utils/live_run_*` runners | | RETAIN (manual) / BROKEN (orchestrator) | Docstrings: "Live API Execution Runner", "Quote API Test Runner", "Data Pipeline Stress Test… verify the local LLM remains stable under concurrent load" (`live_run_orchestrator.py:5-7`) — written against the 122B MoE (`4f44d94`). |
| `ToolManager` registration of `aom`/`maps`/`extractor`/`daemon` as tools | `tool_manager.py:86-100` | BROKEN-FRICTION | Comments "(dangerous — requires HITL approval)" (`:85-99`) tie to `4f44d94` "HITL Bouncer". Why non-runnable classes were registered as tools: UNVERIFIED (no doc). |
