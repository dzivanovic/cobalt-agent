...
## 9. ORIGINAL INTENT (appended 2026-08-21; recorded separately — does not alter §1–§5 verdicts)

Intent sources available (tracked only — vault ADR/PRD notes gitignored and not read; `cobalt_master_context.txt` absent from disk): git commit subjects/bodies; ADR/System-Manifest/Security-Architecture text embedded in `dev_utils/generate_constitution.py`; `dev_utils/create_prd.py` (PRD-001); test/module docstrings naming sprint phases; `COBALT-REQUIREMENTS.md`. Where nothing documents a component, intent is marked UNVERIFIED.

| Component | File | Verdict (from §1/§5) | ORIGINAL INTENT (documentation trail) |
|---|---|---|---|
| `MemoryProvider` ABC + JSON `MemorySystem` | `memory/base.py`, `memory/core.py` | RETAIN / KILL-candidate | Initial agent (`a366e9e` 2026-02-08 "Core agent with Search and Browser tools"; `a55a96a` ReAct loop) — file-backed short/long-term memory; the ABC was added so JSON and Postgres are swappable ("regardless of storage (JSON vs Postgres)", `base.py:3`). Requirements §11#9 now mandates Postgres — JSON fallback intent superseded. |
| `PostgresMemory` core (`memory_logs`, embeddings, `search`) | `postgres.py:546-892` | RETAIN (fix) | `8c5c0c1` (02-09) "added postgres memory and secure docker stack"; `8463874` (02-12) "Level 3 Upgrade — RAG Memory (Postgres+Vector)"; `8c6c5d8` (02-26) "Phase 5 Postgres Vector Librarian"; System Manifest §4 "Memory: PostgreSQL (Vector + Relational), Obsidian (Markdown)" and CoS responsibility "Memory: maintaining the context of all projects and trades" (`generate_constitution.py:121,171`); Requirements §7 "filing/press-release text chunked into pgvector alongside the existing 5-pillar memory schema". |
| OpenAI `text-embedding-3-small` embedder | `postgres.py:243,762` | BROKEN-FRICTION (H-7) | Came with `8463874` RAG upgrade (UNVERIFIED which commit fixed the model). **Conflicts** with documented intent: ADR-002 "DeepSeek-R1 (Local 70B)… Zero data leakage" (`generate_constitution.py:242`) and Requirements §4 local-first. No ADR records choosing a cloud embedder. |
| `_scrub_secrets` before persist/embed | `postgres.py:551-598` | RETAIN intent | Zero-Trust sprint (`5becd1e` 02-24 VaultManager "RAM-only secret injection"; `770c43b` 02-25). Security Architecture §3 "No Hardcoded Keys… loaded into memory only at runtime" (`:198-203`). Specific scrubbing decision: UNVERIFIED (no doc). |
| Graph memory (`graph_nodes`/`graph_edges`, upsert/get) | `postgres.py:621-672,896-1168` | RETAIN (content stale) | `333c2c3` (02-28) "finalize watcher daemon, graph memory, and full documentation"; `4f44d94` (03-06) "Fixed… graph table boot wipes"; `37f51ed` (03-08) "AST graph memory"; test docstring "Entity-Relationship (ER) Graph… Ticker, Material, Strategy" (`tests/test_postgres_graph.py:2-7`). Purpose = entity graph for the Watcher/UniversalExtractor delta alerts (Pass 2). |
| `dev_utils/ingest_knowledge.py` (AST + vault/code ingestion) | | BROKEN-FRICTION | `8c6c5d8` "Postgres Vector Librarian"; `37f51ed` "Upgraded ingest_knowledge.py with AST parsing"; `KnowledgeSearchTool.description` "internal vector database (codebase, playbooks, and Obsidian vault)" (`tools/knowledge.py:13`). |
| `hitl_proposals` runtime DDL + `_hilt_` trio | `postgres.py:674-708,1036-1118` | RETAIN DDL / KILL-candidate trio | `cf9cf4f` (03-01) "persistent HITL approvals" (proposals must survive restarts). The trio vs `HITLProposalStore` duplication: UNVERIFIED (no doc; likely two implementations of the same sprint). |
| `FastPathCache` (`browser_fast_path`, embedding column, ivfflat) | `postgres.py:143-543` | KILL-candidate (embedding half) | `b090110` (02-27) "AOM agentic browser loop, zero-trust security, and pgvector fast path"; test docstring "Fast Path Browser Cache (Phase 3)" (`tests/test_browser_fast_path.py:2`); module docstring "Used for Fast Path cache lookups in Phase 3" (`postgres.py:7`). Intent: reuse deterministic Playwright scripts for repeated tasks (CLAUDE.md "FastPathCache for Playwright script reuse"). The vector-similarity part has no documented rationale — UNVERIFIED. |
| `db/schema.sql` 5-Pillar relational schema | | RETAIN (reconcile) | `cc14caa` (04-07) "5-Pillar schema integration"; `dev_utils/test_5_pillar_db.py` docstring "Sprint 06 — Task 1.9"; `init_5_pillar_schema.py` "ready for Task 2 (Finviz Orchestrator)"; schema comments per pillar (Entities / Physics / Catalysts / Engine / Execution). PRD-001 §4.2 "Math Package" (Ticker, Strategies, Zones, Coefficients) and task 34 "Automated Trade Journaling" explain `trades`/`strategy_signals`/`key_levels`; `trading_accounts`/`order_fills` reflect the pre-requirements era when an execution bridge (Ion/TradeStation, ADR-001) was planned — now out of scope (Requirements §3). |
| `dev_utils/init_5_pillar_schema.py`, `db_status.py`, `test_5_pillar_db.py` | | BROKEN-FRICTION / RETAIN / BROKEN-FRICTION | `cc14caa`; docstrings ("safe database migration… Cortex memory tables are NEVER dropped", "Database Status Audit Utility"). |
| Conn-string builder / `PostgresConfig` | `postgres.py:609-612`, `config.py:102-108,611-615` | BROKEN-FRICTION (H-3) | `705344b` (02-19) "route via POSTGRES_HOST"; `61f300d` Pydantic Settings; `bdd1b5a` (03-01) "db auth translation". No doc for URL-encoding — gap, not intent. |
