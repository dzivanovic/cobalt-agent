...
### Status 2026-09-20
First status block (SESSION-CLOSE step 4a, ruled 09-19 R14). Written by close hub `close-0919` from `reports/ladder-audit-2026-09-19.md`, re-verified against the tree and the three deploy reports (L35); the audit predates all three deploys. Status only — scope, dates and order change by his ruling alone. Legend: DONE-LIVE · DONE-DARK · BUILT-NOT-MERGED · NOT STARTED.
...
| F10 Dots / ladder / accept-up-down (S2-P2) | DONE-LIVE — never rendered | Same deploy, same switch-on, same first render as F8. `card.proposed_key` and `card.curves` = his 09-19 R17 numbers; no `htf_level_proximity` curve shipped (nothing invented). L7 PROMOTION (engine-fed replacing his hand grade) is NOT covered and needs the agreement stats + his HITL (L7/L8, n stated). |
...
| S2 smoke (`cobalt smoke s2`) | NOT STARTED as acceptance | Command is on main (shipped with P4) but was DELIBERATELY NOT RUN at deploy 2 (`deploy-p4-2026-09-19.md` §6.5): K9 is red by design until the first replay row (Mon 21:10 ET) and K3 grades a frozen HOLD red (tribunal A7, ruling owed). The dev-vault day replay is impossible (`cobalt_dev` holds no radar rows; R27 ruled no mirror). Owed: `cobalt smoke s2 --prod --cutoff <P4 deploy instant>` on/after the first replay run (`BACKLOG.md` "S2 close": GREEN = S2 done). |
| Live mornings 1–3 beside DAS | NOT STARTED (0 of 2–3) | R12 target met for premarket Monday 09-21; R13 makes Mon–Wed the shadow run too. Nothing is banked until Monday 04:00 ET. |
| Agent SDK spike (parallel, non-write) | NOT STARTED | Still zero evidence it ever ran: no hit in any report or `BACKLOG.md` beyond the ladder line and the audit. Needs an explicit decision (run before S5, or drop) — his. |

**Stop date: 2026-09-23. AT RISK.** Reason: every ladder feature is now deployed (F2/F8/F10/F3/F12/F13 and the panel), so nothing is left to build inside S2 — but S2's own acceptance is behavioural and none of it has happened: no card has rendered (first render Mon 09-21 04:00 ET), no replay has run (first Mon 09-21 21:10 ET), no live morning is banked, and the smoke cannot be green before that first replay row. The remaining calendar is exactly the three trading mornings Mon–Wed with NO slack, and a single failed morning (a fund-rule or curve surprise on the first live render, or the K3/A7 ruling still owed) puts the acceptance past 09-23. Not LATE: the date has not passed and nothing built is missing. S3 (09-24) also carries two owed preconditions — the DRC template review and stop-override authority (mock #6) — untouched since 09-10.

OFF-LADDER work of 2026-09-19 (ruled, not on a ladder line; each with the ruling that ordered it):
- Three production deploys in one day — 09-18 R3/R5 (weekend push, trading done) and 09-19 R3/R32; approvals R7 (deploy 1, `2893a7f`), R30 (deploy 2, `5b58b7b`), R36 (deploy 3, `851335e`); L43 overruled once, for deploy 3 only (R32). DONE-LIVE: `deploy-2026-09-19`, `-19b`, `-19c`.
- Append-only archiver (`archiver/append-0919`) — R4 (retracted by R5) / R5 / R8 / R25 / R26 / R34 / R35. DONE-LIVE in `upsert` mode with deploy 3 (`851335e`, migrations 0010/0011, R25 REVOKE in force, first nightly Mon 20:30 ET); ONE shadow night before he rules on the numbers (R35); the switch to `append` is his separate ruling, NOT STARTED. Backfill window bounded out of 04:00–09:30 ET (R34 B), ceiling stays 50.
- Ops 0919 items (`ops/2026-09-19`, deploy 3): C2 trading-day guard, trade-count band validator, `lock_timeout` on the migrate transaction, test hygiene A1/A2, migrate output names its code tip, plus the DB-run `SET LOCAL` fix — ordered by 09-18 R1 (list 2) and 09-19 R3/R6, checked by the houses in three rounds. DONE-LIVE.
- L68 stacked pre-merge gate (R19 → L68–L70) — first two real catches: deploy 2 run 1's nine-file conflict (→ R31) and the L53 51.00 rpm overshoot (→ R34). DONE-LIVE as practice.
- R34 C, stagger the pool cadences (screens/lists poll fast though they change slowly) — his "as soon as possible"; NOT STARTED.
- Bars-lifecycle / partitioning tribunal — 09-18 R17, 09-19 R10/R15: Sunday 2026-09-20 13:05 ET, blocks nothing; NOT STARTED (inherits the migration-proof cost: 106.8 s of a 149 s outage).
- Fund rule (R16) and card values (R17/R20) sit inside F13 / F8 / F10 above, not off-ladder.
