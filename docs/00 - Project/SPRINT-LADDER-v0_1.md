...
### Status 2026-09-21
Second status block (SESSION-CLOSE step 4a, ruled 09-19 R14); replaces `### Status 2026-09-20`. Written by close hub `close-0920` from `reports/cto-2026-09-20.md` (§4 R1–R48, §0–§47), the day's check and apply reports, re-verified against the tree (L35): `git tag --list "deploy-2026-09-*"` = no tag after `deploy-2026-09-19c`; `git branch --list "bars/*"` = `bars/chunk-e-0920` `982d958`, `bars/chunk-1a-0920` `6961ee0`, `bars/chunk-2-0920` `1351da6`, all unmerged. What changed since the last block: F10's `htf_level_proximity` curve is LIVE (R35 → R44, 09-20 21:05 ET); R33 / R38 / R27 / R28 / R40 rulings below; the bars-lifecycle work is OFF-LADDER (list at the end). Status only — scope, dates and order change by his ruling alone. Legend: DONE-LIVE · DONE-DARK · BUILT-NOT-MERGED · NOT STARTED.

| Feature / prompt | Status | Evidence |
|---|---|---|
| F2 Radar pool 24×5 (S2-P1) | DONE-LIVE | S2-P1 live 09-13 (`7bca336`); `com.cobalt.radar` resident, restarted by every deploy today (pid 19014 after deploy 3); L13 acceptance run 09-14. |
| F8 Card at precondition (S2-P2) | DONE-LIVE — **never rendered a card** | Code shipped DARK in deploy 1 (`STACK DEPLOY DONE 2893a7f`, tag `deploy-2026-09-19`, 08:25 ET). Switched ON 2026-09-19 12:30:01 ET (`CARDS LIVE 12:30:01 ET · file sha256 f3663399… · stored values: match`, `reports/cards-golive-2026-09-19.md`; R12/R17/R20). No scan runs on a weekend, so **the first render is Monday 2026-09-21 04:00 ET premarket** (R27) = live morning 1 of the L7 shadow (R13). Cards visible and advisory; he grades by hand; the engine's dots recorded beside his. |
| F10 Dots / ladder / accept-up-down (S2-P2) | DONE-LIVE — never rendered | Same deploy, same switch-on, same first render as F8 (Mon 09-21 04:00 ET). `card.proposed_key` and `card.curves` = his 09-19 R17 numbers, leg-count curve `[[0,10],[1,10],[2,6],[3,3],[5,1]]` confirmed as his (09-20 R34). **`htf_level_proximity` curve LIVE 2026-09-20 21:05 ET**: `[[0,10],[1,1]]` (his R35 value — linear, at the level 10, low end one daily ATR; his R44 "Approved"), file sha256 `5993c57f…fbfc17`, the other three curves unchanged, rollback not used (`reports/cards-htf-apply-2026-09-20.md` last line `CARDS HTF LIVE 21:05 ET … stored curve: [[0,10],[1,1]] · other three curves: unchanged · rollback: not used · ESCALATE: 3`; desk read `card.curves` back 21:08). It reaches the dot's number + colour and, on filled cards, a health pill — nothing reaches conviction, `proposed_key` or size. L7 PROMOTION (engine-fed replacing his hand grade) is NOT covered and needs the agreement stats + his HITL (L7/L8, n stated). |
| F3 Focus list, conviction WHY | DONE-LIVE | Rank + WHY row live since S2-P1/S2-P3; pick-vs-rank persistence (`"user".picks`) shipped with S2-P4: `P4 DEPLOY DONE 5b58b7b`, tag `deploy-2026-09-19b`, 15:17 ET, migrations 0008/0009. DRC rendering stays F14/S3. |
| Radar panel (§5 host) — S2-P3 | S2-P3 DONE-LIVE; **§5 acceptance NOT STARTED** | `sprint-2/radar-panel` is fully merged: `git log main..sprint-2/radar-panel` is EMPTY, tip `ebb1231` = the 09-15 merge (`cto-2026-09-15.md` §6.5 step 3), tag `deploy-2026-09-15`. It is NOT BUILT-NOT-MERGED — the close prompt's premise did not survive the check. Read-only `/radar` pool view live since 09-15; the card slice went live with F8/F10 on 09-19. The acceptance clause (2–3 live mornings beside DAS on the panel) has **0 of 2–3 banked**: Mon 09-21 · Tue 09-22 · Wed 09-23 remain. |
| F12 Missed records (nightly replay) — S2-P4 | DONE-LIVE — never run | Deploy 2 (`5b58b7b`, `deploy-2026-09-19b`): `com.cobalt.replay` bootstrapped (`state = not running`, loaded; `RunAtLoad false`, Mon–Fri 21:10 ET); **first run Monday 2026-09-21 21:10 ET**. Its first job row is also what turns smoke K9.x from red-by-design to checkable. |
| F13 Auto-archive + benchmark + miss line — S2-P4 | DONE-LIVE — never run | Same deploy / same first run. Fund rule R16 (`Asset Type` non-blank OR `Industry` = `Exchange Traded Fund`) changes what the live radar drops from Monday 04:00 ET premarket — its first live proof. |
| S2-P1 / S2-P2 / S2-P3 / S2-P4 (code prompts) | DONE-LIVE ×4 | P1 09-13; P2 dark 09-19 08:25 (`2893a7f`) then live 12:30 ET; P3 09-15 (`ebb1231`, `deploy-2026-09-15`); P4 09-19 15:17 (`5b58b7b`). P4's own `OWED` items (A2 deadline scope, A7/K3, ESCALATE 19) are carried in the close report, not ladder scope. |
| S2 smoke (`cobalt smoke s2`) | NOT STARTED as acceptance | Command is on main (shipped with P4) but was DELIBERATELY NOT RUN at deploy 2 (`deploy-p4-2026-09-19.md` §6.5): K9 is red by design until the first replay row (Mon 21:10 ET) and K3 grades a frozen HOLD red. The dev-vault day replay is impossible (`cobalt_dev` holds no radar rows; 09-19 R27 ruled no mirror). Owed: `cobalt smoke s2 --prod --cutoff <P4 deploy instant>` on/after the first replay run (`BACKLOG.md` "S2 close": GREEN = S2 done). **Reading rule, ruled 09-20 R32 (18:23 ET "A"):** a legitimate frozen HOLD grading K3 red is an ACCEPTED loud false positive (tribunal A7); the reader tells a HOLD from a RETAIN defect with the `degraded_sources` procedure in K3's `expect_text`; no discriminator column is built. A red K3 that the hand-check shows to be a HOLD does NOT make the S2 smoke red as a verdict — the S2-close report states the hand-check and its result beside the K3 line. |
| Live mornings 1–3 beside DAS | NOT STARTED (0 of 2–3) | R12 target met for premarket Monday 09-21; 09-19 R13 makes Mon–Wed the shadow run too. **09-20 R33 (18:25 ET "A"):** the mornings Mon 09-21 · Tue 09-22 · Wed 09-23 COUNT toward the "2–3 live mornings" from Monday, independent of `cobalt smoke s2`; the smoke (K3 read per R32) stays a separate, required gate on S2 done (sets aside only the ordering word "then" in the S2 smoke paragraph, a plan line). Nothing is banked until Monday 04:00 ET. |
| Agent SDK spike (parallel, non-write) | NOT STARTED — decided: KEPT | Still zero evidence it ever ran. **09-20 R39 (18:41 ET "A"):** kept; a hub first RE-SCOPES it to what desk/hub practice since 09-15 has not already answered, then it runs as a non-write night lane during S3 (09-24 → 10-07), blocking nothing, so S5's F20 does not start cold. No hub has re-scoped it yet. |

**Stop date: 2026-09-23. AT RISK — unchanged.** Reason, same as the last block: every ladder feature is deployed (F2/F8/F10/F3/F12/F13 and the panel), so nothing is left to build inside S2 — but S2's own acceptance is behavioural and none of it has happened: no card has rendered (first render Mon 09-21 04:00 ET, with the `htf_level_proximity` curve now live in it), no replay has run (first Mon 09-21 21:10 ET), 0 of 2–3 live mornings banked, and the smoke cannot be green before that first replay row. **ZERO days elapsed of the three remaining** (Mon 09-21 · Tue 09-22 · Wed 09-23), NO slack: a single failed morning puts the acceptance past 09-23. Not LATE: the date has not passed and nothing built is missing. Two rulings of 09-20 shorten what could have gone wrong at the close: R33 (the mornings count from Monday, independent of the smoke) and R32 (a K3 red that is a HOLD is an accepted loud false positive, read with the hand-check). S3 (09-24) precondition DRC template review — untouched since 09-10 — is still owed; the other S3 precondition, stop-override authority (mock #6), is RULED (09-20 R38, below). Monday's clock, once: 04:00 ET first card render + shadow morning 1 · 20:30 ET the archiver's first nightly (the ONE shadow night, 09-19 R35, `shadow_compare` on per 09-20 R29) · 21:10 ET first replay run → `cobalt smoke s2 --prod` becomes checkable.
