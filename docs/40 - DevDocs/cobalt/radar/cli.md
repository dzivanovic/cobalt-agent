...
2026-09-21 (setups one build STEP-1): `radar evaluate` gains `--expect-formed` (a `store_true` flag). It is valid only with `--replay` and `--trade-def`. When the replay forms 0 times it exits non-zero (FINAL §9 gate 4).
