...
**2026-09-22 — X11 (STEP-4).** Four more named reasons: `range.micro.touches_per_side_unset`, `range.micro.touch_tolerance_atr_unset`, `range.micro.bound_flat_slope_atr_unset` and `leg.consolidation_max_retrace_unset`. Each is the null-row reason of a D2 / D3 detector key. No atom spelling changed, and every Range(micro) / Leg(opening_drive) atom validates as a bare atom.

**2026-09-22 — X11 (STEP-8).** Two more named reasons: `range_break.failed_trap_bars_unset` and `range_break.retest_tolerance_atr_unset`. `event(retest)` and `event(stop_hit)` validate as bare event atoms.
