...
**2026-09-22 (fix round 3, F1 — R47, Grok's fix).** `detect_extension` takes an optional keyword `direction`. None (every existing caller) is today's rule, byte for byte: the direction is the run's sign. A given direction is the frame's side binding: the Extension of that direction is read whatever the run's sign, and path B counts only a distance moved in that direction. The frame passes `direction="down"` only for a def whose long-side text names a state past the culmination (`frame.binds_side_by_frame`); Rubberband keeps the detector's own direction (FINAL fact 3). Fable's fix (a direction stamped at the culminating bar and held) is not built. X10 now PASSES.
