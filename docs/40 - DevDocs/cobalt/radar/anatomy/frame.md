...
**2026-09-22 (fix round 3, F3 — R49).** The lazy `pullback_roles` object reads `leg_roles.min_size(tunables)` (`leg.min_size_atr`): null → `pullback_roles(legs)` exactly as before; a value → `pullback_roles(legs, min_size=…, atr=<the seeded ATR>)`. A role atom whose roles are `unavailable` (a set minimum, no ATR yet) reads `unavailable` with that reason.
