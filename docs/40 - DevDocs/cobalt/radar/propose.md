...
`screens validate` shares the same derivation path, hashes current note bytes, validates the supplied pool and actual/prospective Lists, checks the ruled total-transport request budget (pool + screens + list chunks, not pool alone), and compares prose-derived fields with installed YAML. It performs no network, DB, vault, or proposal write.
