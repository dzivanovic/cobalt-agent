...
A screen note's own column declaration keeps being parsed here, because the note's formatting is this module's business: `_columns()` strips backticks and whitespace, answers the `<same columns>` placeholder with `None`, and hands whatever is left to `config.screener_columns` — the radar's one column-set source (AT-1 2.3, L3). A declaration that function refuses becomes the same `ProposalRefused` as before, chained to the config error. The `ft` comparison probe's one-column `c: "0"` is a literal and deliberately not a column set.
