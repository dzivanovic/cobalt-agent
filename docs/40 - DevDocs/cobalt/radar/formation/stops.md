...
**2026-09-22 — `turn_candle` (STEP-8; `A-23`).** `STRUCTURAL_REFS[turn_candle]` is the low of the RangeBreak sequence's turn bar. `structural_extreme` applies its buffer and nudge. No turn yet → `InsufficientBars`.
