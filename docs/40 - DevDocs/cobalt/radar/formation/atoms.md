...
**2026-09-22 (fix round 3, F3 — R49).** The five leg-role atoms (`Leg(pullback).direction` / `.end` / `.index`, `Leg(impulse).direction`, `Leg(opening_drive OR impulse).direction`) declare `tunable_keys = leg_roles.ROLE_TUNABLE_KEYS` (`leg.min_size_atr`, `A-24`), so the key is in the declared closure of every def that names them (nine-ema-scalp and vwap-continuation in the corpus). `_ROLE_REASONS` gains `insufficient_seed` (a set minimum before the ATR has a value).
