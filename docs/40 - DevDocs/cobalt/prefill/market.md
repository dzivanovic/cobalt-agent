...
FAILED. VIX and BTC are explicitly NOT servable by this endpoint
(`UNSERVABLE` dict) — neither is a Finviz stock/screener ticker;
callers render "n/a (manual)" for those two.

## Key functions/classes
- `MarketRow` — frozen dataclass: ticker, price (Decimal), change_pct (Decimal).
- `MARKET_TICKERS = ("SPY", "QQQ", "IWM")`.
- `UNSERVABLE` — `{"VIX": "...", "BTC": "..."}`, the reason string daily.py splits on.
