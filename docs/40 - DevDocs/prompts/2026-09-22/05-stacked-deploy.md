...
- **P12 PRODUCTION BASELINE**, each its own call, verbatim:
  - `COBALT_ENV=production uv run cobalt heartbeat show` → the BASELINE.
    ...
    - A RED on the aset/sheet probe or the radar probe → `FAILED PREFLIGHT: <probe> already red — the deploy could not be judged` — EXCEPT the one named case below.
    - [fold, desk 11:2x after the 11:06 `FAILED PREFLIGHT` — the 09-21 R21 / R25 per-ticker degraded design] A radar probe RED whose findings are ONLY `failed_stage bars: poll failures: <n>` plus zero or more per-record findings of the shape `poll <ticker> <reason> since <ts>` (the probe adds one per carried record older than `heartbeat.radar_scan_max_age_s`, `probes.py:110-117`) — no `last_scan_at stale`, no `degraded …`, no `mirror …`, no `poll failure <ticker> has invalid since`, and no `failed_stage bars:` with any detail OTHER than `poll failures: <n>` (a lifecycle refusal stamps `failed_stage bars: lifecycle …`, `runner.py:305`, and IS the stop) on that line, `com.cobalt.radar` OK with `heartbeat fresh` — is a CARRIED PER-TICKER BARS-POLL FAILURE (`src/cobalt/radar/runner.py:376`: the runner stamps `failed_stage='bars'` while ≥1 per-ticker failure record is carried; `poller.py:67`: a record is popped only by a successful fetch of its own ticker or when the ticker leaves membership). It says nothing about the radar's ability to be judged. It is BASELINE: name it verbatim, record `<n>` as `<pf0>`, it is not a stop — PROVIDED IT HOLDS ON A SECOND READ [fold, house read 12:08 — Gemini, HOLDS]: one heartbeat beat can sample the pool row between S2's carried-record stamp (`runner.py:209-218`) and the same cycle's lifecycle check (`runner.py:251-311`), so a single reading cannot tell a carried per-ticker failure from a lifecycle refusal that has not stamped yet. Therefore: run `COBALT_ENV=production uv run cobalt heartbeat show` AGAIN as the FIRST row of STEP-3 (≥100 s after this one by `date` — STEP-2's suites take minutes; never a wait command) and quote it under `## PREFLIGHT` as `P12 second read`: the radar line must show the SAME finding kind (only `failed_stage bars: poll failures: <n>` plus per-record `poll …` findings) — any other finding on the second read → `FAILED PREFLIGHT: radar probe already red — the deploy could not be judged (second read: <line>)`, before anything is down. Any OTHER radar finding on that line, or the sheet probe RED, is the stop above.
...
- (e) `COBALT_ENV=production uv run cobalt heartbeat show` → no RED that was not in P12's baseline. The known replay RED is baseline and clears at 21:10. A RED stamped inside the outage window is an exception: name it. [fold, desk 11:2x] The radar probe's carried per-ticker line, if P12 recorded `<pf0>`, is still baseline here whatever `<n>` now reads and whatever `poll <ticker> <reason> since <ts>` per-record findings it carries (records are carried, aged and dropped per ticker by the radar's own polls, not by this deploy — the radar is never restarted by this set); the row is RED for THIS deploy only if the radar line carries a finding of ANOTHER kind (`last_scan_at stale`, `degraded …`, `mirror …`, `poll failure <ticker> has invalid since`, or a `failed_stage bars:` detail other than `poll failures: <n>`) or `com.cobalt.radar` is not `running … heartbeat fresh` — and this row is read TWICE [fold, house read 12:08 — Gemini, HOLDS: a beat sampled between S2's stamp and the cycle's lifecycle check is not proof]: once here, and again as the LAST smoke row after (g), at least 100 s apart by `date` (fewer than 100 s → run (d), (f) and (g) once more first; never a wait command); the radar line's finding kind must be the same on both reads, else the row is RED.
