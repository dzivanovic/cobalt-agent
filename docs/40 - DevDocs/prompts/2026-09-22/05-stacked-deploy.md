...
(i) Main = `deploy/stacked-0922`'s tip, so it was MERGED. Bootstrap whichever of `<restart set>` is not loaded, with 4.4's calls, then 4.5. This holds at any hour.
(ii) Main is neither `<pre-merge>` nor the gate tip, so a revert was interrupted. Go to STEP-5 (2) at once, with the revert re-derived from `git -C /Users/cobalt/cobalt log --oneline -3`.
...
- (b) THE LOG CHECK, tied to this start:
  ...
  - `tail -n 8 /Users/cobalt/cobalt/logs/radar.err` → a `radar cycle:` line stamped at or after `<t down>`: the radar kept cycling through the window, or came back. None yet → run (c)–(e) first, then this tail again. At most THREE tails in total, never a wait command. None after three → RED.
- (c) The curls:
...
- (0) `date`. This path is allowed at any hour today. R3 set aside the window clause, and 2.7 proved that nothing the archiver, replay or backup imports is in this set.
