...
- (e) `COBALT_ENV=production uv run cobalt heartbeat show` → no RED that was not in P12's baseline. The known replay RED is baseline and clears at 21:10. A RED stamped inside the outage window is an exception: name it. [fold, desk 11:2x] The radar probe's carried per-ticker line, if P12 recorded `<pf0>`, is still baseline here whatever `<n>` now reads (records are carried and dropped per ticker by the radar's own polls, not by this deploy — the radar is never restarted by this set); the row is RED for THIS deploy only if the radar line carries a finding of ANOTHER kind (`last_scan_at stale`, `degraded …`, `mirror …`) or `com.cobalt.radar` is not `running … heartbeat fresh`.
