...
- **THIS LAUNCH** is the desk's row **R__R** of `cto-2026-09-23.md` (the desk replaces the token `R__R` in this file with its row number before launch; `R__A` / `R__L` below are `07`'s own placeholders, quoted as text and never filled in this file). An unfilled token makes the grep below print nothing, which is the mismatch: `grep -n "^| R__R " "/Users/cobalt/cobalt/docs/40 - DevDocs/reports/cto-2026-09-23.md"` must name `08-review-stacked-deploy.md`; `git -C /Users/cobalt/cobalt log -1 --format=%H -S"08-review-stacked-deploy.md" -- "docs/40 - DevDocs/reports/cto-2026-09-23.md"` must be NON-EMPTY.
