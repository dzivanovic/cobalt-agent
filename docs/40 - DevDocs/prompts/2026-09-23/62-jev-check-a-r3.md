...
DESK: watch the report's LAST NON-BLANK LINE (L71), `^(JEV CHECK A R3 DONE|FAILED)`. Launch row: **R76**. The desk fills it; the row must carry the literal `62-jev-check-a-r3.md CHECK A R3` and the stagger literal `no other house hub is running`.
