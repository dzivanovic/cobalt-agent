...
- **P6 THE TWO CHECKS** (L67). Each file must be COMMITTED: `git -C /Users/cobalt/cobalt log -1 --format=%H -- "<path>"` NON-EMPTY.
  ...
  - `tail -n 3` of the smoke-fix check report the desk's R__L row names — ROUND 2 if it ran (`reports/s2-smoke-fix-check-r2-2026-09-23.md`, `prompts/2026-09-23/17-s2-smoke-fix-check-r2.md`), else round 1 (`reports/s2-smoke-fix-check-2026-09-23.md`); the R__L row quotes its path and stop line. The LAST NON-BLANK line is `06`'s DONE shape (a `round: 2` field allowed), with `ready … YES` from at least THREE houses and `defects that HOLD: 0` — OR, HIS OVERRIDE `cto-2026-09-23.md` §4 **R26** ("ship", 08:1x ET): `defects that HOLD: 0`, grok and opus `ready … YES`, gemini `ready … NO`, and the R__L row quotes R26 and that line → ACCEPTED, record `P6 smoke: R26 override` — OR, if fewer than three houses could check (METER / HARNESS), the L67 floor (≥1 house other than Anthropic) with the desk's R__L row naming that count as his accepted floor for this check. A HOLD > 0 is a stop unless a committed row of HIS in `<desk file>` ships with it by name.
